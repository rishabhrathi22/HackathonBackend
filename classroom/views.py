...
from .serializers import ClassroomSerialzier, CreateClassroomSerializer
...
class ClassroomViewSet(viewsets.GenericViewSet):
    ...
    default_serializer_class = ClassroomSerialzier
    model = Classroom
    ...
    def list(self, request):
        ...
        serializer = ClassroomSerialzier(queryset, many=True)
    ...
    def retrieve(self, request, standard,section,subject):
        try:
            ...
            serializer = ClassroomSerialzier(classroom)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except:
